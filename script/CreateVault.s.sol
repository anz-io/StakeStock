...
contract CreateVault is Script {
    function run() external {
        ...
        address initialOwner = vm.parseAddress(vm.prompt("Initial Owner Address (who will manage the vault):"));

        string memory timelockInput =
            vm.prompt("Initial Timelock in seconds (e.g., 86400 for 1 day, 0 for no timelock):");
        uint256 initialTimelock = vm.parseUint(timelockInput);

        address asset = vm.parseAddress(vm.prompt("Asset Address (e.g., mUSDC - the token users will deposit):"));
        ...
        string memory saltInput = vm.prompt("Salt (leave empty for random, or provide hex string):");
        bytes32 salt;
        ...
        require(keccak256(bytes(confirm)) == keccak256(bytes("yes")), "Vault creation cancelled");
        ...
        // Create vault
        IMetaMorphoV1_1 vault = factory.createMetaMorpho(initialOwner, initialTimelock, asset, name, symbol, salt);
